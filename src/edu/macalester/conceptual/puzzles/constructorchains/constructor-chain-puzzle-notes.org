:PROPERTIES:
:ID:       20260701T101910.577011
:END:
#+TITLE: COMP 127 conceptual puzzles: constructor chains

* notes from Paul 2025-11-25

** FIXME TODO ensure that each puzzle requires you to really go up the entire chain

idea: first generate the actual chain, in which each class extends the
parent. Then come back later and sprinkle in detractors: look at a
class, and decide whether to add a sibling, or a child.

Factor out code for a single class declaration? You'd need to send list
of superclasses and one it will extend.
...
* source code → AST generation code
How to go from source code to the javaparser API calls that create the
same code?
...
Use the output to inform the code below that builds your AST; the AST is
built bottom-up, and the text output uses indentation for the tree, so
"bottom-up" here means "rightmost-indented left" there, so to speak
...
instead. It has one main trick and exactly one other thing going on—one
...
:PROPERTIES:
:CREATED:  [2025-11-13T14:28:31-0600]
:END:
What dials can you turn to affect the output to make the puzzle harder
or easier?

- Depth :: how deep the inheritance hierarchy goes.
- Number of siblings :: at a given level in the hierarchy, how many
  sibling classes are created?
- Print statement in default ctor :: in the default constructor, do we
  print "<classname> default constructor"?
- Non-default constructor :: does the class have a constructor that
  takes a single int parameter?
- Print statement in non-default ctor :: in any given non-default
  constructor, do we print "<classname> constructor, n = <n>"?
- Object creation :: in a given constructor (default or otherwise), is
  there an object created? (Type is a superclass of the current one.)

Considered but not done:

- Method call :: in the current class or ancestor thereof, include a
  method that prints or returns a value; in a constructor, call that in
  a constructor. This seems to be more about inheritance and overrides
  than object creation and constructors, though.

* distractors?
Right now the generated puzzle uses every class it outputs, and the
hierarchy is linear.

Earlier the plan was to generate some distractors -- class declarations
that just never get used. But it's not clear that having them
meaningfully exercises the students' conceptual understanding.

We could use the distractors (which makes them...not distractors any
more?) for object creation statements, but then handling the topology of
the hierarchy (so that we don't create an object whose type is a
subclass and throws us into an infinite loop) is more complex. The juice
does not seem worth the squeeze, I think.
* [2026-07-01 Wed] returning to this trying to get it merged soon
:PROPERTIES:
:ID:       20260701T101928.699643
:END:
